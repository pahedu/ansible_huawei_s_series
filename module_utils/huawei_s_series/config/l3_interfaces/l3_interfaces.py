...
"""
The huawei_s_l3_interfaces class
...
from ansible.module_utils.network.huawei_s_series.utils.utils import validate_n_expand_ipv4, validate_ipv6
    ...
    def verify_diff_again(self, want, have):
        ...
        for each in want:
            ...
            for every in have:
                ...
                if each_want.get('address') != every_have.get('address') and len(each_want.keys()) == len(
                        every_have.keys()):
                    diff = True
                    break
                elif each_want.get('address') != every_have.get('address') and \
    ...
    def _set_config(self, want, have, module):
        ...
        if want.get('ipv4'):
            ...
            if ipv4:
                for each in ipv4:
                    ipv4_dict = dict(each)
                    if ipv4_dict.get('address') != 'dhcp' and not ipv4_dict.get('secondary'):
                        cmd = "ip address {0}".format(ipv4_dict['address'])
                        primary_address_dhcp = False
                    if ipv4_dict.get('address') != 'dhcp' and ipv4_dict.get("secondary") and not primary_address_dhcp:
                        cmd = "ip address {0} sub".format(ipv4_dict['address'])
                    elif ipv4_dict.get('address') != 'dhcp' and ipv4_dict.get("secondary") and primary_address_dhcp:
                        continue
        ...
        if have.get('ipv4') and want.get('ipv4'):
            for each in have.get('ipv4'):
                if each.get('secondary'):
                    cmd = 'ip address {0} sub'.format(each.get('address'))
